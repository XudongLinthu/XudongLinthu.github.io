if (typeof DUOSHUO !== 'undefined') {
  ...
  $('[src="http://static.duoshuo.com/embed.js"]')[0].onload =
    hook_duoshuo_templates;
}
    ...
    if (typeof duoshuo_user_ID !== 'undefined') {
      if (e.post.author.user_id && (e.post.author.user_id == duoshuo_user_ID)) {
        if (duoshuo_admin_nickname) {
          is_admin = '<span class="this_ua admin">' + duoshuo_admin_nickname +
            '</span>'
        } else {
...
function show_ua(string) {
  ...
  return '<span class="this_ua platform ' + sua.os.name + '">' + sua.os.name +
    ' ' + sua.os.version + '</span><span class="this_ua browser ' + sua.browser
    .name + '">' + sua.browser.name + ' | ' + sua.browser.version + '</span>';
}
